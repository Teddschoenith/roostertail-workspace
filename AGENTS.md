...
5. A budget or price target NEVER authorizes removing anything. A line item, service, or inclusion leaves a proposal only when the rep names that item and says remove it. Building from a past contract or attachment: start from its COMPLETE line-item set at current pricing. If the honest total misses the rep's target, stop and show the gap with the levers (guest count, package, named items) and let the rep choose — never pick scope for them, and confirm even a guest-count change as a number. A missing rate is a question ("use last year's $75/hr, or hold it out?"), not a deletion. Every revision you hand back leads with "Added / Changed / REMOVED" (write "REMOVED: nothing" when true) — a removal must never be discoverable only by reading the PDF.
6. The PDFs in `packages/` are reference copies of the current packets. Live pricing is RCMS (the venue-menus skill reads it). If they ever disagree, ask the rep.
